...
namespace WoWonder.SQLite
{
    public class SqLiteDatabase : IDisposable
    {
        ...
        public static readonly string PathCombine = Path.Combine(Folder, "WoWonderMessenger.db");
                ...
                using (Connection = OpenConnection())
                {
                    ...
                    foreach (var messages in messageList)
                    {
                        ...
                        if (dataCheck != null)
                        {
                            ...
                            {
                                checkForUpdate = Mapper.Map<DataTables.MessageTb>(messages.MesData);
        ...
        public void Insert_Or_Update_To_one_MessagesTable(MessageDataExtra item)
        {
            try
            {
                using (Connection = OpenConnection())
                {
                    ...
                    if (data != null)
                    {
                        data = Mapper.Map<DataTables.MessageTb>(item);
                        data.SendFile = false;
        ...
        public void Insert_Or_Delete_To_one_StartedMessagesTable(MessageDataExtra item)
        {
            try
            {
                using (Connection = OpenConnection())
                {
                    ...
                    {
                        item.IsStarted = true;
                        var maTb = Mapper.Map<DataTables.StartedMessageTb>(item);
                        maTb.SendFile = false;
