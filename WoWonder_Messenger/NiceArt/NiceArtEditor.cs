    ...
    public class NiceArtEditor : Java.Lang.Object, INiceArt.IBrushViewChangeListener, INiceArt.IOnGestureControl, INiceArt.IOnSaveBitmap, INiceArt.IOnMultiTouchListener
    {
        ...
        public View GetLayout(ViewType viewType)
        {
            try
            {
                ...
                {
                    rootView = MLayoutInflater.Inflate(Resource.Layout.view_niceart_text, null);
                ...
                {
                    rootView = MLayoutInflater.Inflate(Resource.Layout.view_niceart_image, null);
                }
        ...
        public class MyAsyncTask : AsyncTask<string, string, string>
        {
            public string ImagePath;
            public NiceArtEditorView ParentView;
            public NiceArtEditor Editor;
            public INiceArt.IOnSaveListener OnSaveListener;
            public SaveType Type;
            public SaveSettings SaveSettings;
            public Bitmap SaveBitmap;
            public Bitmap SavedResultBitmap;
            ...
            protected override string RunInBackground(string[] str)
            {
                try
                {
                    ...
                    try
                    {
                        ...
                        stream.Flush();
                        stream.Close();
                        Log.Debug(Tag, "Filed Saved Successfully");
                        return "";
                    }
                    catch (FileNotFoundException ex)
                    {
                       Methods.DisplayReportResultTrack(ex.Message + " \n  " + ex.StackTrace);
                        //ex.PrintStackTrace();
                        Log.Debug(Tag, "Failed to save File");
                        return ex.Message;
                    }
                }
                catch (Exception e)
