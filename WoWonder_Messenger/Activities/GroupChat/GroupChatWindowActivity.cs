...
namespace WoWonder.Activities.GroupChat
{
    ...
    public class GroupChatWindowActivity : AppCompatActivity, MaterialDialog.IListCallback, MaterialDialog.ISingleButtonCallback, View.IOnLayoutChangeListener
    {
        ...
        private void SetRecyclerViewAdapters()
        {
            try
            {
                MAdapter = new MessageAdapter(this, GroupId , true);
            ...
            try
            {
                ...
                if (item != null)
                {
                    switch (e.Type)
                    {
                        ...
                        {
                            var fileName = item.MesData.Media.Split('/').Last();
                            var mediaFile = WoWonderTools.GetFile(GroupId, Methods.Path.FolderDcimVideo, fileName, item.MesData.Media);
        ...
        private void SuggestionAdapterOnOnItemClick(object sender, AdapterClickEvents e)
        {
            try
            {
                ...
                if (Methods.CheckConnectivity())
                {
                    ...
                    {
                        GroupMessageController.SendMessageTask(this, GroupId, time2, SuggestionAdapter.GetItem(position).RealMessage).ConfigureAwait(false);
                    });
                }
                else
                {
                ...
                else if (requestCode == 513 && resultCode == Result.Ok) // right_video camera 
                {
                    if (Methods.MultiMedia.CheckFileIfExits(IntentController.CurrentVideoPath) != "File Dont Exists" && Build.VERSION.SdkInt <= BuildVersionCodes.OMr1)
                    {
                        ...
                        if (Methods.CheckConnectivity())
                        {
                            Task.Factory.StartNew(() =>
                            {
                                GroupMessageController.SendMessageTask(this, GroupId, time2, EmojIconEditTextView.Text, "", IntentController.CurrentVideoPath).ConfigureAwait(false);
                            });
                        }
                        else
                        {
                    ...
                    if (!string.IsNullOrEmpty(placeLatLng))
                    {
                        ...
                        if (latLng?.Length > 0)
                        {
                            ...
                            if (Methods.CheckConnectivity())
                            {
                                Task.Factory.StartNew(() =>
                                {
                                    GroupMessageController.SendMessageTask(this, GroupId, time2, EmojIconEditTextView.Text, "", "", "", "", "",lat, lng).ConfigureAwait(false);
                                });
                            }
                            else
                            {
                            ...
                            { 
                                ...
                                foreach (var item in from item in result.Data.Messages let check = MAdapter.DifferList.FirstOrDefault(a => a.MesData.Id == item.Id) where check == null select item)
                                {
                                    ...
                                    MAdapter.DifferList.Add(new AdapterModelsClassMessage()
                                    {
                                        ...
                                        MesData = await WoWonderTools.MessageFilter(GroupId, item, type)
                                    });
                                }
            ...
            try
            {
                if (TaskWork == "Working")
                {
                    ...
                    if (Methods.CheckConnectivity())
                    {
                        ...
                        (int apiStatus, var respond) = await RequestsAsync.GroupChat.FetchGroupChatMessages(GroupId, "0", "0");
                        if (apiStatus == 200)
                        {
                            int countList = MAdapter.DifferList.Count;
                            if (respond is GroupMessagesObject result)
                                ...
                                if (respondList > 0)
                                {
                                    ...
                                    foreach (var item in result.Data.Messages)
                                    {
                                        ...
                                        else if (check.MesData.Seen == "0" && check.MesData.Seen != item.Seen)
                                        {
                                            ...
                                            check.MesData = await WoWonderTools.MessageFilter(GroupId, item, type, true);
                                            check.TypeView = type;
        ...
        public async void Update_One_Messeges(MessageData message)
        {
            try
            {
                ...
                if (checker != null)
                {
                    ...
                    checker.MesData = await WoWonderTools.MessageFilter(GroupId, message, type);
